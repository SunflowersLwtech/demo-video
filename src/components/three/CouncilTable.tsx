...
import { TABLE_RADIUS, TABLE_HEIGHT } from "../../constants/agents";

export const CouncilTable: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const time = frame / fps;

  // Glow pulse (replaces useFrame ref mutation)
  const glowOpacity = 0.06 + Math.sin(time * 0.8) * 0.02;

  // Rings rotation (replaces useFrame ref mutation)
  const ringsRotation = time * 0.08;
  ...
  return (
    <group position={[0, TABLE_HEIGHT, 0]}>
      ...
      <group
        position={[0, 0.04, 0]}
        rotation={[-Math.PI / 2, 0, ringsRotation]}
      >
        {ringRadii.map((r, i) => (
          <mesh key={i}>
